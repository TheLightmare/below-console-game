...
private:
    std::unordered_map<std::string, std::function<std::unique_ptr<VkScene>()>> scene_factories;
    std::unordered_map<std::string, std::unique_ptr<VkScene>> persistent_scenes;  // Cached scenes
    std::unordered_set<std::string> persistent_scene_names;  // Which scenes to keep alive
    VkScene* current_scene = nullptr;
    std::string current_scene_name;
    void* user_data = nullptr;  // Optional user data propagated to scenes

public:
    VkSceneManager() : current_scene(nullptr), current_scene_name("") {}
    ...
    template<typename T>
    void register_scene(const std::string& name, std::function<std::unique_ptr<VkScene>()> factory) {
